...
public class Alcancia {
    ...
    public String darEstadoAlcancia() {
        ...
        if(this.estado==1){
            return "La alacancia ya esta rota";
        }else{
...
public int obtenerDenominacionMasNumerosa() {
    if (this.numeroMonedas50 + this.numeroMonedas100 + this.numeroMonedas200 + 
            this.numeroMonedas500 + this.numeroMonedas1000 == 0) {
        return 0; // Retornar 0 si no hay monedas
    }
    // Inicializar la cantidad mayor y la denominación correspondiente
    int cantidadMayor = this.numeroMonedas50;
    int denominacionMayor = 50;

    // Comparar con la cantidad de monedas de 100
    if (this.numeroMonedas100 > cantidadMayor) {
        cantidadMayor = this.numeroMonedas100;
        denominacionMayor = 100;
    }

    // Comparar con la cantidad de monedas de 200
    if (this.numeroMonedas200 > cantidadMayor) {
        cantidadMayor = this.numeroMonedas200;
        denominacionMayor = 200;
    }

    // Comparar con la cantidad de monedas de 500
    if (this.numeroMonedas500 > cantidadMayor) {
        cantidadMayor = this.numeroMonedas500;
        denominacionMayor = 500;
    }

    // Comparar con la cantidad de monedas de 1000
    if (this.numeroMonedas1000 > cantidadMayor) {
        cantidadMayor = this.numeroMonedas1000;
        denominacionMayor = 1000;
    }
    
    // Retornar la denominación de la cantidad más numerosa
    return denominacionMayor;
}
    ...
    public boolean valiosa() {
        return this.numeroMonedas50 + this.numeroMonedas100 + this.numeroMonedas200 == 0 && this.numeroMonedas500+
                this.numeroMonedas1000>0; // Retornar 0 si no hay monedas
    }
}
